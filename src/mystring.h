...
class myString {
    public:
        void *my_memchr(const void *str, int c, size_t n); // | strtok !
        ...
        int my_strncmp(const char *str1, const char *str2, size_t n); // Сравнение строк по n символов | strstr !
...
class work_string: public myString {
    private:
        char* str;
    public:
        work_string(char* strin): str(strin){
        }
        work_string(): str(NULL){
        }
        void cat_string(char*); //cat
        void change_string(char*);//cpy
        void compare_string(char*);//cmp
        void output_word_string(char*); //tok
};